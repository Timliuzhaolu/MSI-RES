...
class SRData(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lr, hr, scale = self._get_patch(lr, hr)
        lr, hr = common.set_channel([lr, hr], self.args.n_colors)
        lr_tensor, hr_tensor = common.np2Tensor([lr, hr], self.args.rgb_range)
        return lr_tensor, hr_tensor, filename, scale
    ...
    def _get_patch(self, lr, hr):
        patch_size = self.args.patch_size if self.args.patch_size != -1 else tuple(self.args.patch_sizes)
        if isinstance(patch_size, int):
            assert patch_size > 0
        elif isinstance(patch_size, tuple):
            assert patch_size[0] > 0 and patch_size[1] > 0
        else:
            raise TypeError("Invalid patch_size or patch_sizes")
        ...
        if self.train:
            ...
            if self.args.augment:
                if isinstance(patch_size, int):
                    ...
                    lr, hr = common.augment([lr, hr], rot=False) # Rotation with non-square patches is messy
